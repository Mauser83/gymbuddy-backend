import { verifyRoles, verifyGymScope, verifyPremiumAccess } from '../../../src/modules/auth/auth.roles';
import { AuthContext, GymRole, AppRole, UserRole } from '../../../src/modules/auth/auth.types';
...
describe('auth.roles', () => {
  ...
  test('verifyRoles throws when requirements not met', () => {
    expect(() => verifyRoles(baseContext, { requireAppRole: AppRole.ADMIN })).toThrow();
  });

  test('verifyRoles allows OR conditions', () => {
    const ctx = { ...baseContext, appRole: AppRole.ADMIN } as AuthContext;
    expect(() => verifyRoles(ctx, { or: [{ requireAppRole: AppRole.ADMIN }, { requireUserRole: UserRole.PREMIUM_USER }] })).not.toThrow();
  });
  ...
  test('verifyGymScope uses permissionService', () => {
    ...
    const ctx = { ...baseContext, gymRoles: [{ gymId: 1, role: GymRole.GYM_ADMIN }] } as AuthContext;
    expect(() => verifyGymScope(ctx, permissionService as any, 1)).not.toThrow();
  ...
  test('verifyGymScope throws when not allowed', () => {
    ...
    expect(() => verifyGymScope(baseContext, permissionService as any, 1)).toThrow();
  });
