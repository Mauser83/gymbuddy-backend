...
describe('MediaService.presignGetForKey', () => {
  ...
  it('signs private global key', async () => {
    const url = await svc.presignGetForKey(`private/global/equipment/1/approved/${UUID}.jpg`, 60);
    expect(url).toContain('https://signed-url.example');
  });

  it('signs global upload key', async () => {
    const url = await svc.presignGetForKey(`private/uploads/global/2/2025/01/${UUID}.jpg`, 60);
    expect(url).toContain('https://signed-url.example');
  });
  ...
  it('rejects invalid prefix', async () => {
    await expect(svc.presignGetForKey('some/other/prefix/file.jpg', 60)).rejects.toThrow(
...
describe('MediaService.imageUrl', () => {
  ...
  it('clamps ttl and returns expiresAt', async () => {
    const now = Date.now();
    const out = await svc.imageUrl(`public/golden/1/2025/01/${UUID}.jpg`, 9999, 1);
  ...
  it('HEADs private keys and throws when missing', async () => {
    (svc as any).s3.send = jest.fn().mockRejectedValue({ $metadata: { httpStatusCode: 404 } });
    await expect(
...
describe('MediaService.getImageUploadUrl', () => {
  ...
  it('returns url, key, and required headers', async () => {
    ...
    expect(out.requiredHeaders).toEqual([{ name: 'Content-Type', value: 'image/jpeg' }]);
  });
