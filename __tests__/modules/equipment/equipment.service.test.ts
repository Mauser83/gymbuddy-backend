...
describe('EquipmentService', () => {
  ...
  test('createEquipment validates, checks roles and creates equipment', async () => {
    ...
    expect(prisma.equipment.create).toHaveBeenCalledWith({ data: { ...input } });
    expect(result).toEqual({ id: 1 });
  });

  test('createEquipment propagates verifyRoles error', async () => {
    mockedVerify.mockImplementation(() => { throw new Error('no'); });
  ...
  test('getEquipment fetches by id', async () => {
    ...
    expect(prisma.equipment.findUnique).toHaveBeenCalledWith({ where: { id: 2 } });
  ...
  test('uploadEquipmentImage validates, checks roles and creates image', async () => {
    ...
    const input = { equipmentId: 1, url: 'u' } as any;
  ...
  test('createEquipmentCategory validates and creates', async () => {
    ...
    expect(prisma.equipmentCategory.create).toHaveBeenCalledWith({ data: input });
  });
  ...
  test('updateEquipmentCategory validates and updates', async () => {
    ...
    expect(mockedValidate).toHaveBeenCalledWith({ name: 'n', slug: 's' }, UpdateEquipmentCategoryDto);
    expect(prisma.equipmentCategory.update).toHaveBeenCalledWith({ where: { id: 1 }, data: { name: 'n', slug: 's' } });
  });
  ...
  test('deleteEquipmentCategory checks roles and deletes', async () => {
    ...
    expect(mockedVerify).toHaveBeenCalled();
    expect(prisma.equipmentCategory.delete).toHaveBeenCalledWith({ where: { id: 4 } });
  });
  ...
  test('createEquipmentSubcategory validates and creates', async () => {
    ...
    expect(prisma.equipmentSubcategory.create).toHaveBeenCalledWith({ data: input });
  });
  ...
  test('updateEquipmentSubcategory validates and updates', async () => {
    ...
    expect(prisma.equipmentSubcategory.update).toHaveBeenCalledWith({ where: { id: 1 }, data: input });
  });
  ...
  test('deleteEquipmentSubcategory checks roles and deletes', async () => {
    ...
    expect(mockedVerify).toHaveBeenCalled();
    expect(prisma.equipmentSubcategory.delete).toHaveBeenCalledWith({ where: { id: 5 } });
  });
});