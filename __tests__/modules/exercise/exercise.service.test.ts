...
describe('ExerciseService', () => {
  ...
  test('createExercise validates, checks role and creates slots/options', async () => {
    prisma.user.findUnique.mockResolvedValue({ userRole: 'PREMIUM_USER' } as any);
    ...
    const input: any = {
      ...
      difficultyId: 1,
      exerciseTypeId: 2,
      equipmentSlots: [{ slotIndex: 0, isRequired: true, comment: 'c', options: [{ subcategoryId: 3 }] }],
    };
    const res = await service.createExercise(input, 1);
  ...
  test('createExercise throws when user not premium', async () => {
    prisma.user.findUnique.mockResolvedValue({ userRole: 'USER' } as any);
    await expect(service.createExercise({} as any, 1)).rejects.toThrow('Upgrade to premium to create exercises');
  });

  test('getExercises forwards parameters to prisma', async () => {
    prisma.exercise.findMany.mockResolvedValue([] as any);
    await service.getExercises('a', { exerciseType: ['t'], difficulty: ['d'], bodyPart: ['b'], muscle: ['m'] });
  ...
  test('getExerciseById returns result', async () => {
    ...
    expect(prisma.exercise.findUnique).toHaveBeenCalledWith(expect.objectContaining({ where: { id: 1 } }));
    expect(res).toEqual({ id: 1 });
  });
  ...
  test('updateExercise handles equipment slots', async () => {
    ...
    const input: any = { equipmentSlots: [{ slotIndex: 0, isRequired: true, options: [{ subcategoryId: 2 }] }] };
  ...
  test('updateExercise throws when unauthorized', async () => {
    ...
    await expect(service.updateExercise(1, {} as any, 1)).rejects.toThrow('Unauthorized exercise access');
  });

  test('deleteExercise soft deletes after ownership check', async () => {
    prisma.exercise.findUnique.mockResolvedValue({ userId: 1 } as any);
    await service.deleteExercise(3, 1);
    expect(prisma.exercise.update).toHaveBeenCalledWith({ where: { id: 3 }, data: { deletedAt: expect.any(Date) } });
  });
  ...
  test('deleteExerciseType deletes type', async () => {
    ...
    expect(prisma.exerciseType.delete).toHaveBeenCalledWith({ where: { id: 2 } });
  });
  ...
  test('createExerciseDifficulty validates and creates', async () => {
    ...
    expect(prisma.exerciseDifficulty.create).toHaveBeenCalledWith({ data: input });
  });
  ...
  test('updateExerciseDifficulty validates and updates', async () => {
    ...
    expect(prisma.exerciseDifficulty.update).toHaveBeenCalledWith({ where: { id: 1 }, data: input });
  });

  test('deleteExerciseDifficulty deletes', async () => {
    await service.deleteExerciseDifficulty(1);
    expect(prisma.exerciseDifficulty.delete).toHaveBeenCalledWith({ where: { id: 1 } });
  });
  ...
  test('updateBodyPart validates and updates', async () => {
    ...
    expect(prisma.bodyPart.update).toHaveBeenCalledWith({ where: { id: 1 }, data: input });
  });
  ...
  test('updateMuscle validates and updates', async () => {
    ...
    expect(prisma.muscle.update).toHaveBeenCalledWith({ where: { id: 1 }, data: input });
  });
  ...
  test('updateMetric validates and updates', async () => {
    ...
    expect(prisma.metric.update).toHaveBeenCalledWith({ where: { id: 1 }, data: input });
  });
