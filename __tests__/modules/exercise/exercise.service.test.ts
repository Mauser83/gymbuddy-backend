...
describe("ExerciseService", () => {
  ...
  test("createExercise propagates verifyRoles error", async () => {
    ...
    await expect(
      service.createExercise({} as any, {} as any, 1)
    ).rejects.toThrow("no");
  ...
  test("getExercisesAvailableAtGym queries exercises when equipment exists", async () => {
    prisma.gymEquipment.findMany.mockResolvedValue([
      { equipment: { subcategoryId: 5 } },
    ] as any);
    prisma.exerciseEquipmentOption.findMany.mockResolvedValue([
      { slot: { exerciseId: 2 } },
    ] as any);
  ...
  test("updateExercise handles equipment slots", async () => {
    ...
    const input: any = {
      equipmentSlots: [
        { slotIndex: 0, isRequired: true, options: [{ subcategoryId: 2 }] },
      ],
    };
  ...
  test("deleteExercise throws when unauthorized", async () => {
    prisma.exercise.findUnique.mockResolvedValue({ userId: 2 } as any);
    await expect(service.deleteExercise(3, 1)).rejects.toThrow(
      "Unauthorized exercise access"
    );
  });
