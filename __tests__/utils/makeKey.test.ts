...
describe("makeKey", () => {
  ...
  it("creates global upload key with equipmentId", () => {
    ...
    expect(key).toBe(
      "private/uploads/global/9/2025/01/123e4567-e89b-4a12-9abc-1234567890ab.jpg",
    );
    expect(isValidStorageKey(key)).toBe(true);
  });
  ...
  it("rejects missing or invalid IDs", () => {
    ...
    expect(() => makeKey("upload_global", { equipmentId: 0 }, { now: FIXED })).toThrow(/positive integer/);
    expect(() => makeKey("upload", { gymId: -2 }, { now: FIXED })).toThrow(/positive integer/);
  });

  it("rejects invalid ext and uuid", () => {
    // @ts-expect-error
    expect(() => makeKey("upload", { gymId: 1 }, { now: FIXED, ext: "gif" })).toThrow(/jpg, png, webp/);
    expect(() =>
      makeKey("upload", { gymId: 1 }, { now: FIXED, uuid: "not-a-uuid" })
    ).toThrow(/UUID v4/);
  });
});
